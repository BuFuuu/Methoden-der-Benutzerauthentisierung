...
static inline void precompute_outer(__m128i* W) {
...
/* set result from right pwd candidate
 * W[0] holds current four pwd candidates
 * res defines the right one */

static inline int found(int * res, char* result, __m128i W0, uint32_t* W1) {
	uint32_t *val = (uint32_t*) &W0;
	switch(*res) {
		case 0xf: *val = val[0]; break;
		case 0xf0: *val = val[1]; break;
		case 0xf00: *val = val[2]; break;
		default: *val = val[3]; break;
	}
...
int crackHash(struct state hash, char *result) {
	...
	/* we don't use W[2]-W[14] so we don't have to set them to zero
	 * W[0], W[1], W[15] are initialized with their values */
	...
	/* define constants needed for generating all possible permutations with repetition of W[0] */

	const __m128i pwd_step = _mm_set_epi32(4,4,4,4);
	const __m128i pwd_overflow3 = _mm_set_epi32(234,234,234,234);
	const __m128i pwd_overflow2 = _mm_set_epi32(4,4,234,234);
	const __m128i pwd_overflow1 = _mm_set_epi32(234,234,4,4);
	int g;	
	int h;	
	int i;
	...
	/* naive approach to generate all W[1] which have 2 not fixed chars with using pointer
	...
	for (bPtrW[2]=0x61; bPtrW[2]<=0x7a; ++bPtrW[2]) {
		for (bPtrW[3]=0x61; bPtrW[3]<=0x7a; ++bPtrW[3]) {		
			...
			/* set initial values for a new generation of all possible permutations of W[0] */

			W[0] = _mm_set_epi32(0x6161607a,0x61616079,0x61616078,0x61616077);

			/* The permutations of W[0] are generated by taking four inital values 'aaaa', 'baaa', 'caaa', 'daaa'
			 * and (in case they didn't lead to the desired hash) adding 4 to generate four new pwd candidates
			 * when the values have to carryover e.g. round t = 'waaa' => round t+1 = 'abaa' we add 234 instead.
			 * after both carryover cases are handled we generate until we reach 'wbaa', 'xbaa', 'ybaa', 'zbaa'
			 * as we are now aligned again we can start again with 'caaa' etc. and repeat these steps.
			 * when reaching 'wzaa' - 'zzaa' we need to add a constant value again to get to 'aaba' - 'daba'
			 * but the principle is the same (even easier as we don't have to handle 2 different carryovers)
			 * it generally works because 26^4 is divisble by 4 without rest */ 
			...
			while (--g) {
				...
				while (--h) {
					i = 14;
					while (--i) {
						W[0] = _mm_add_epi32(W[0],pwd_overflow3);
						if (sha1_hashBlock(W, dgst, res)) return found(res, result, W[0], pwd);
						W[0] = _mm_add_epi32(W[0],pwd_step);
						if (sha1_hashBlock(W, dgst, res)) return found(res, result, W[0], pwd);
						W[0] = _mm_add_epi32(W[0],pwd_step);
						if (sha1_hashBlock(W, dgst, res)) return found(res, result, W[0], pwd);
						W[0] = _mm_add_epi32(W[0],pwd_step);
						...
						W[0] = _mm_add_epi32(W[0],pwd_step);
						if (sha1_hashBlock(W, dgst, res)) return found(res, result, W[0], pwd);
						W[0] = _mm_add_epi32(W[0],pwd_overflow1);
						if (sha1_hashBlock(W, dgst, res)) return found(res, result, W[0], pwd);
						W[0] = _mm_add_epi32(W[0],pwd_overflow2);
						if (sha1_hashBlock(W, dgst, res)) return found(res, result, W[0], pwd);
						W[0] = _mm_add_epi32(W[0],pwd_step);
						if (sha1_hashBlock(W, dgst, res)) return found(res, result, W[0], pwd);
						W[0] = _mm_add_epi32(W[0],pwd_step);
					...
					W[0] = _mm_add_epi32(W[0],_mm_set1_epi32(58880));
				}
				W[0] = _mm_add_epi32(W[0],_mm_set1_epi32(15073280));
			}
		}
	}
    /* Not found */
